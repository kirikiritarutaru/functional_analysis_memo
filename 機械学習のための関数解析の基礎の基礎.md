...
- 関数解析の基礎（とくに，ヒルベルト空間）をまとめる
- 機械学習を見通しよく理解することを目的とする
- 集合・位相，線形代数，微積，確率に関する基本的な内容は既知であると仮定する

---

## 関数解析とは？

- 抽象的な線形空間に展開される「無限次元の線形代数」

---
## なぜ関数解析か

- 「機械学習」という分野は様々な手法のごった煮である
- 自分たちの目の前にある問題を解決するためには，まず原理・原則を身に着け，統一的な視点から各手法を眺めることが必要になる（[参考pdf][yamada_pdf]）

---

## 機械学習×関数解析を理解するためのサブゴール
...
- 線形空間を$\mathbf{V}$とする．
- バナッハ空間を$\mathcal{B}$とする．
- ヒルベルト空間を$\mathcal{R}$とする．
...
**[Def]**
$\mathbf{S}$を$\mathbf{V}$の部分集合とする．$\mathbf{S}$が$\mathbf{V}$の中の演算に関して再び線形空間になっているとき，$\mathbf{S}$を$\mathbf{V}$の部分空間という．
...
2. 線形空間に対して幾何学的な視点を持つことができる

便利な場面の具体例として，本スライドでは以下をあげる

- 1 に対してはニューラルネットワークの普遍性定理
- 2 に対しては標本化定理
...
<!-- TODO. この後の議論にはリース-マルコフ-角谷の表現定理が必要 -->
...
という形をした$\mathbb{R}^d$上の関数全体の集合$\mathcal{G}$は，$\mathbb{R}^d$上の実数値連続関数全体の集合$C(\mathbb{R}^d)$で稠密になる．
すなわち，任意の$f \in C(\mathbb{R}^d)$と任意の正数$\epsilon$に対して，上の形で表現される関数$G$を適当にとれば，
$$ \forall x \in \mathbb{R}^d, \quad |f(x)-G(x)|< \epsilon $$
が成立する．
...
**[Prf]**
背理法で示す．$\exist f \in C(\mathbb{R}^d) \setminus \overline{\mathcal{G}}$．ハーン・バナッハの拡張定理より，$\psi_{\overline{\mathcal{G}}}=0$かつ$\psi(f) \not ={0}$となる有界線形汎関数$\psi:C(\mathbb{R}^d) \rightarrow \mathbb{R}$をとることができる．しかしながら，リースの表現定理により
