...
$$ \sigma(\lambda(\langle x,y\rangle +\theta)+\phi) = \left\{
   \begin{aligned}
   &\rightarrow 1\ (\lambda \rightarrow \infty として\langle x,y\rangle +\theta>0の場合)\\
   &\rightarrow 0\ (\lambda \rightarrow \infty として\langle x,y\rangle +\theta<0の場合)\\
   &\rightarrow \sigma(\phi)\ (任意の\lambdaに対して,\ \langle x,y \rangle+\theta = 0の場合)
  \end{aligned}\right.$$
となる。
