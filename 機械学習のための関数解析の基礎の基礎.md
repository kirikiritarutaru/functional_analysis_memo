...
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
ここで、 $w\in \mathbb{R^n}$ を固定し、 $\mathbb{R}$ 上の有界な可測関数 $h$ に対して、線形汎関数 $F$ を
...
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
$L^\infty(\mathbb{R})$ において有限の値をもつ単関数は稠密であること、すなわち、零集合を除いて 任意の $f\in L^\infty(\mathbb{R})$ に一様収束する単関数の列 $\{s_n\}$ がとれることを示す。
ルベーグ積分の基本のキのスライド「単関数列と可測関数」より、任意の $|f|$ に収束する単関数の列 $\{s_n\}$ がとれることがわかる。 $|f|$ は有界なので、上記スライドより単関数列 $\{s_n\}$ は $f$ に一様収束する。よって、任意の $\varepsilon >0$ に対しある $n_0 \in \mathbb{N}$ が存在し、
$$|f| \le |\text{esssup} (f)| \subset (|f-s_n| \le \varepsilon)\quad (\forall n \ge n_0)$$
が成り立つ。したがって、
$$\mu((\varepsilon < |f-s_n-)) \le \mu (|\text{esssup}(f)| \le |f|)=0 \quad (\forall n \ge n_0)$$
以上より、 $\mathbb{R}$ 上の 任意の有界な可測関数 $h$ に対して、 $F(h)=0$ となることがわかる。
