...
4. $(ab)*f = a*(b*f) \quad (b \in \mathbb{R}\ \text{or}\ \mathbb{C})$
5. $1*f = f$
...
任意の$\mu \in M(X)$に対して、
$$ \forall y\in \mathbb{R}^n,\ \forall\theta \in \mathbb{R},\ \int_X \sigma(\langle x, y\rangle + \theta)d\mu=0 \Rightarrow \mu=0$$
が成り立つ。
...
任意の $x,y\in \mathbb{R}^n,\ \theta,\phi \in \mathbb{R}$ に対して、
$$ \sigma(\lambda(\langle x,y\rangle +\theta)+\phi) \rightarrow \left\{
   \begin{aligned}
   &\ 1\ (\lambda \rightarrow \infty として\langle x,y\rangle +\theta>0の場合)\\
   &\ 0\ (\lambda \rightarrow \infty として\langle x,y\rangle +\theta<0の場合)\\
   &\ \sigma(\phi)\ (任意の \lambda に対して,\ \langle x,y \rangle+\theta = 0 の場合)
  \end{aligned}\right.$$
となる。
...
ここで、 次の記号を定義する。
$$\begin{aligned}
  \Pi_{y,\theta} &= \left\{x \in X \mid \langle x,y \rangle +\theta =0 \right\} \\
  H_{y,\theta} &= \left\{x \in X \mid \langle x,y \rangle +\theta >0 \right\}
\end{aligned}$$

また、$\sigma_\lambda(x) = \sigma(\lambda(\langle x,y \rangle + \theta)+\phi)$ は $\lambda \rightarrow \infty$ のとき、
$$\gamma(x) = \left\{\begin{aligned}
  1 & \qquad (\langle x,y \rangle +\theta >0) \\
  0 & \qquad (\langle x,y \rangle +\theta <0) \\
  \sigma(\phi) & \qquad (\langle x,y \rangle +\theta =0)
\end{aligned}\right.$$
に各点収束する。
...
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
ルベーグの優収束定理（の特別なケース）より、
任意の $\phi, \theta,y$ に対して、$\lambda \rightarrow \infty$ のとき
$$\begin{aligned}
...
\end{aligned}$$
が成り立つ。
($\langle x,y\rangle +\theta$ が $0$ のときと $0$ より大きい場合で場合分け)

---
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
ここで、$y$ を固定し、$\mathbb{R}$ 上の有界な可測関数 $h$ に対して、線形汎関数 $F$ を
$$F(h) = \int_X h(\langle x,y \rangle)d\mu$$
により定める。$\mu$ が有限な符号付き測度であるため、$F$ は $L^\infty(\mathbb{R})$ 上の有界な関数である。

---
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
$h$ を区間 $[0,\infty)$ の特性関数 $\chi_{[0,\infty)}$ とした場合、
$$F(h) = \int_X h(\langle x,y \rangle)d\mu = \mu(\Pi_{y,-\theta}) + \mu(H_{y,-\theta})=0$$
となる。同様に $h$ を区間 $(0,\infty)$ の特性関数 $\chi_{(0,\infty)}$ とした場合も、$F(h)=0$ となる。

$F$ の線形性により、$F(h)$ は任意の区間の特性関数 $h$ に対して、$F(h)=0$ になる。また、これらより有限個の区間の定義関数 $h_1,h_2,\dots$ の和 $h_s=h_1+h_2+\cdots$ についても $F(h_s)=0$ になる。

---
## 連続なシグモイド型関数はdiscriminatory関数
**[Prf]** つづき
$A=\{\langle x,y \rangle \mid x\in X\}$ は $\mathbb{R}$ の有界閉区間。A上の任意の連続関数 $g$ は $\mathbb{R}$ の区間の特性関数の線型結合 $\sum^N_{k=1} a_k \chi_{J_k}$ により一様近似可能。すなわち、任意の正数 $\epsilon$ について、適切に $a_k,J_k$ をとれば、
$$\forall x\in A,\ \left|g(x) - \sum^N_{k=1} a_k\chi_{J_k}(x) \right|$$
とできる。
