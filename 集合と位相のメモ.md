...
[N.B.]
$X$ のいろんな覆い方が存在するが、その覆い方の中から有限個選ぶだけで、 $X$ を覆うことができる。すべての覆い方の中に、その有限個の覆い方が存在するという強い主張。
...
となる。
次に、 $f(x)=1$ とする。このときは $0<r=\frac{n}{2^m}< \varepsilon$ となる $r$ をとると $x\in X-\overline{U_{1-r}}$ かつ $X-\overline{U_{1-r}} \ni y \Rightarrow f(y) \ge 1-r$ となる。
したがって、
...
**[Prf 4]**
最後に、 $0<f(x)<1$ とする。このときは、 $f(x) -\varepsilon < r=\frac{n}{2^m} < f(x) <r^\prime = \frac{n^\prime}{2^{m^\prime}} < f(x) + \varepsilon$ となる $r,r^\prime$ をとれば、 $x \in U_{r^\prime} - \overline{U_r}$ かつ $U_{r^\prime} - \overline{U_r} \ni y \Rightarrow r \le f(y) \le r^\prime$ となる。
したがって、
...
次の文字を定義する。
$$ \begin{aligned}
C(X) &= \left\{u\ \middle|\ u は X 上連続な関数 \right\} \\
...
$X$ を局所コンパクトハウスドルフ空間とする。
$O_1,O_2,\dots,O_n$ を $X$ の開集合、 $K$ を $X$ のコンパクト集合とする。 $O_1,O_2,\dots,O_n$ によって、 $K$ が被覆されるならば、すなわち、 $K \subset O_1 \cup O_2 \cup \cdots \cup O_n$ ならば、以下の条件をみたす連続関数 $u_j\ (j=1,2,\dots,n)$ が存在する。
1. $u_j \prec O_j \qquad (j=1,2,\dots,n)$
2. $u_1(x)+u_2(x)+\cdots+u_n(x) = 1 \qquad (x\in K)$
...
が存在する。
このとき、 $K_j = \bigcup_i \left\{\overline{V_{x_i}}\ \middle|\ \overline{V_{x_i}} \subset O_j \right\}$ とおけば、 $K_j$ が求めるものである。
...
$$u_1+u_2+\cdots+u_n=1-(1-v_1)(1-v_2)\cdots(1-v_n)$$
