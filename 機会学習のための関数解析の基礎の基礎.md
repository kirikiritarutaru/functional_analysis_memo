...
**[Prf]**
背理法で示す．$\exist f \in C(\mathbb{R}^d) \setminus \overline{\mathcal{G}}$．ハーン・バナッハの拡張定理より，$\psi_{\overline{\mathcal{G}}}=0$かつ$\psi(f) \not ={0}$となる有界線形汎関数$\psi:C(\mathbb{R}^d) \rightarrow \mathbb{R}$をとることができる．しかしながら，リースの表現定理により$\forall h \in \mathcal{G}, \psi(h)=\langle h,v_f \rangle=0$となる$v_f$が存在．$v_f=0$しか存在しないので，矛盾．よって，定理の主張は示された．← おかしい
